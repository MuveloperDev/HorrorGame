Shader "Unlit/UI_Dissolve_Edge_Effect"
{
    Properties
    {
        ...
        */
        ...
        Pass
        {
            ...
            v2f vert(appdata_t IN)
            {
                ...
                // 객체의 월드 좌표를 클립 좌표로 변환합니다.
                OUT.vertex = UnityObjectToClipPos(IN.vertex);

                // 텍스처 좌표를 설정합니다.
                OUT.texcoord = IN.texcoord;
            
                // 정점 색상과 _Color를 곱하여 최종 색상을 계산합니다.
                OUT.color = IN.color * _Color;

                // xy: Noise 텍스처의 UV, z: Dissolve 인자, w: width
                // UV1의 x값을 언팩하여 effectFactor로 설정합니다.
                OUT.effectFactor = UnpackToVec4(IN.uv1.x);

                // xyz: 색상, w: softness
                // UV1의 y값을 언팩하여 effectFactor2로 설정합니다.
            ...
            fixed4 frag(v2f IN) : SV_Target
            {
                ...
                // 엣지 감지
                fixed edgeDetect = step(0.0, factor) - step(0.1, factor); // 0.0 ~ 0.1 사이의 값을 엣지로 감지

                 // 엣지 색상 변경

                color.rgb = lerp(color.rgb, IN.effectFactor2.rgb, edgeDetect); // 엣지 색상 적용
